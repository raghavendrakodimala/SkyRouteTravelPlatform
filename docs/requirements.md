...
#### Authentication and Authorisation Extensibility (v1.2; extended v1.4 — identity protocol breadth)
...
| DP-AUTH-004 | Authorisation policies (what a user is allowed to do) must be expressible independently of authentication schemes (how identity is proved). When auth is introduced in a future sprint, named ASP.NET Core authorisation policies (e.g., `"RequireBookingOwner"`) must be the mechanism — not inline identity checks inside service methods. This requirement is a design constraint for the MVP implementation: no service method may contain logic that will need to be replaced by a policy. See DP-POLICY-001–003, which generalise this pattern to any pluggable business/eligibility policy, not authorisation alone. | OCP, SRP | Must Have |
| DP-AUTH-005 | The `AuthService` seam (DP-AUTH-003) and the ASP.NET Core authentication-pipeline seam (DP-AUTH-001, DP-AUTH-002) must be demonstrably capable of hosting any standard identity protocol — including OpenID Connect (OIDC), OAuth 2.0, SAML 2.0, and enterprise Single Sign-On (SSO) — without changing business logic, service classes, or Angular components. Concretely: introducing an OIDC/OAuth 2.0 provider (e.g., via `Microsoft.AspNetCore.Authentication.OpenIdConnect` or JWT bearer authentication), a SAML 2.0 provider (e.g., via a SAML middleware library), or an SSO broker must require only: (a) new authentication middleware registration in `Program.cs`, (b) a new `AuthService` implementation on the Angular frontend, and (c) named authorisation policy definitions (DP-AUTH-004, DP-POLICY-001). No controller, service class, or component may require modification to support any of these protocols. | OCP, DIP | Must Have |
| DP-AUTH-006 | Identity claims or attributes obtained from any of the protocols named in DP-AUTH-005 (OIDC claims, SAML assertions, SSO-provided attributes) must be normalised at the authentication-pipeline boundary into the plain domain scalars already required by DP-AUTH-001 (e.g., `string userId`, `string tenantId`) before reaching any service method. No service method may need to know which identity protocol produced the caller's identity. | OCP, SRP | Must Have |

Note on YAGNI: No auth middleware, auth library, policy class, or token handler is required in the MVP. No OIDC client library, OAuth 2.0 client/server implementation, SAML library, or SSO broker integration is added or configured in the MVP. BR-010 (no authentication or authorisation required for MVP) is unchanged. These requirements constrain what must NOT be done (no scheme-specific branching, no security principal in service signatures, no embedded auth logic in components, no protocol-specific knowledge inside service methods) — they do not require any new implementation beyond the no-op `AuthService` class on the frontend.

Verifiability: at code review, confirm DP-AUTH-001 and DP-AUTH-002 hold (no `ClaimsPrincipal`/`IIdentity`/scheme-name checks in service or repository code) and that exactly one `AuthService` class exists on the frontend (DP-AUTH-003). If these hold, addability of OIDC, OAuth 2.0, SAML, or SSO (DP-AUTH-005) is structurally guaranteed — the protocol-specific integration itself is only exercisable when a real provider is introduced, at which point this handoff's completion criteria (no controller/service/component changes) becomes the acceptance test.
...
#### Multi-Database Support (v1.2; strengthened v1.4 — configuration-externalised database connectivity)
...
| DP-DB-005 | Database connection configuration (connection strings, provider selection, credentials) must be sourced exclusively through the standard .NET configuration abstraction (`IConfiguration`, environment variables, or `appsettings.{Environment}.json`) — never hardcoded in source code. When a real database is introduced, switching between a relational database, a document database, or a managed cloud-hosted database service must require only a new `IBookingStore` implementation, an updated DI registration, and updated configuration — consistent with DP-PERSIST-004, DP-CLOUD-003, and DP-CLOUD-005. This confirms and strengthens the database-agnosticism guarantee already established by DP-PERSIST-001–005 and DP-DB-001–004. | DIP, OCP | Must Have |
...
#### Framework and Dependency Upgrade Flexibility (v1.4)

Angular and .NET both ship major versions on a predictable cadence (Angular roughly every six months; .NET annually, alternating STS/LTS). If domain or service-layer code couples directly to framework internals, undocumented behaviour, or third-party library-specific types, a major-version upgrade can force a broad rewrite instead of an isolated adapter change. This section constrains where framework- and library-specific types may appear so that a future upgrade — or a like-for-like library swap — touches only adapter/infrastructure code.

| ID | Requirement | Principle | Priority |
|---|---|---|---|
| DP-UPGRADE-001 | No domain model, service interface, or service implementation class may reference a third-party library type in its public signature (e.g., a specific validation library's result type, a specific HTTP client library's request/response types). Only .NET BCL types and the framework-standard abstractions already permitted elsewhere in this document (e.g., `Microsoft.Extensions.Logging.ILogger<T>`) may appear in service signatures. | OCP, DIP | Must Have |
| DP-UPGRADE-002 | Angular component and service code must use Angular's public, documented APIs (standalone components, the Angular DI system, documented control-flow syntax) rather than internal/private Angular APIs or undocumented behaviour. Public APIs carry Angular's own deprecation and migration guarantees across major versions; internal APIs do not. | Maintainability | Should Have |
| DP-UPGRADE-003 | Backend code must use ASP.NET Core's documented hosting, dependency-injection, and middleware APIs (the `Program.cs` minimal hosting model, `IServiceCollection`, `IApplicationBuilder`) rather than reflection-based workarounds or undocumented internals. This keeps the codebase aligned with the officially supported upgrade path between .NET major versions. | Maintainability | Should Have |
| DP-UPGRADE-004 | Framework and library version numbers must be defined in exactly one location per ecosystem: `package.json` for the Angular/Node dependency tree, and the `.csproj` file(s) for .NET/NuGet dependencies. No version number may be hardcoded or duplicated in source code, comments, or other configuration files outside these manifests. | DRY | Must Have |

Note on YAGNI: no automated dependency-upgrade tooling (e.g., Dependabot, Renovate), no framework-abstraction/compatibility-shim layer, and no multi-version support matrix is required or implemented in the MVP. No upgrade is actually performed as part of this requirement. This group only constrains where framework- and library-specific types may appear in code so that a future upgrade is an adapter-layer change, not a domain/service-layer rewrite.

Verifiability: at code review, inspect domain model and service-layer files for `using` statements referencing third-party (non-BCL, non-`Microsoft.Extensions.*`) namespaces in public signatures. Inspect `Program.cs` and configuration files for hardcoded version strings duplicated outside `package.json`/`.csproj`.

#### Pluggable Policy Extension Point (v1.4)

DP-AUTH-004 already requires that authorisation be expressed as named ASP.NET Core authorisation policies rather than inline identity checks. This section generalises that requirement: the same named-policy pattern must apply to any future business/eligibility rule that a client may want to customise or override — not authorisation rules alone.

| ID | Requirement | Principle | Priority |
|---|---|---|---|
| DP-POLICY-001 | Any conditional rule that could plausibly become client-configurable or pluggable in the future (authorisation rules, eligibility rules, booking-window rules, or similar business gating logic) must not be implemented as inline branching scattered across multiple service methods. Each such rule must be expressed as a single, independently named policy check (a named method, a named ASP.NET Core authorisation policy per DP-AUTH-004, or a named predicate/delegate) so that a future policy source can add, remove, or override one named policy without touching unrelated code. This extends DP-AUTH-004 from "authorisation policies" to a general policy-extension point. | OCP, SRP | Must Have |
| DP-POLICY-002 | Service methods that must evaluate a policy must depend on a policy-evaluation seam (e.g., an injected `IAuthorizationService`, or an injected policy-evaluation delegate/interface) rather than evaluating the policy's criteria inline in the method body. In the MVP, no non-trivial policy exists (BR-010: no auth), so no real policy-evaluation implementation is required — the constraint is only that if a policy check is introduced later, it is added as a call to a named seam, not as new inline conditional code inserted into existing method bodies. | DIP, OCP | Must Have |
| DP-POLICY-003 | Each named policy must be independently identifiable, independently testable, and independently toggleable — a future policy source (configuration file, admin UI, or policy engine) must be able to change one named policy without affecting any other policy or any unrelated service logic. | SRP, Testability | Must Have |

Note on YAGNI: no generalised policy engine, no configuration-driven policy store, no policy administration UI, and no runtime policy-override mechanism is required or implemented in the MVP. BR-010 (no authentication or authorisation required) is unchanged. This group only requires that if and when a custom policy is introduced, it is introduced as a named, isolated seam rather than as inline conditional logic mixed into existing service methods.

Verifiability: at code review, search service method bodies for multi-branch `if`/`switch` logic that encodes an eligibility or authorisation-style rule directly rather than delegating to a named policy check or authorisation policy attribute. Any such inline rule is a candidate violation.

#### Zero Trust Architecture Readiness (v1.4)

BR-010 establishes that no authentication or authorisation is required for any MVP endpoint. This does not conflict with the requirements below: zero trust is a set of structural properties (no implicit trust by network location, independent verification per request, least-privilege interface scoping, no hardcoded inter-component trust) that the architecture must not preclude, even while auth itself remains switched off for MVP. These requirements are informed by NIST SP 800-207 (Zero Trust Architecture).

| ID | Requirement | Principle | Priority |
|---|---|---|---|
| DP-ZEROTRUST-001 | No controller, service, or frontend code may treat a caller as trusted based on network origin, IP address range, or deployment topology (e.g., "internal-network calls skip validation"). Input validation (FR-060–FR-065) must apply uniformly regardless of the caller's network location, and this uniformity must never be bypassed for a specific caller or network segment. | Security | Must Have |
| DP-ZEROTRUST-002 | No backend component may bypass the defined interface boundaries (DP-001–DP-004) to reach another component's internals directly (e.g., a provider must never be given direct access to `IBookingStore`; a controller must never reach into another controller's state). Every cross-component interaction must go through an explicit, named interface, so that per-call verification can later be layered onto these boundaries without introducing new coupling. | DIP, SRP | Must Have |
| DP-ZEROTRUST-003 | The authentication/authorisation seam (DP-AUTH-001–DP-AUTH-006) must support per-request evaluation — identity and policy checks are evaluated on every request through the ASP.NET Core pipeline, never cached or assumed valid for the lifetime of a session or connection. This is consistent with, and reinforced by, the request-statelessness requirement already established in DP-PROTOCOL-003b. | Security, SRP | Must Have |
| DP-ZEROTRUST-004 | Every interface introduced in this document must expose only the operations its actual consumer needs (least privilege) — for example, `IBookingStore` exposes only the operations `BookingService` requires (DP-PERSIST-005), not broader data access. This principle must be preserved for every new interface added in future work, including any interface introduced to satisfy DP-CLOUD-001. | SRP, DIP | Must Have |
| DP-ZEROTRUST-005 | No configuration value or code path may encode a hardcoded trust shortcut — an allow-listed IP range that skips validation, a shared bypass token, or a "trusted header" that is accepted without independent verification. If a trusted-header pattern is ever introduced for an internal call, it must be explicitly documented and independently verifiable, not silently trusted. | Security | Must Have |

Note on YAGNI: no mutual TLS, no service mesh, no per-request cryptographic attestation, and no policy enforcement point/policy decision point (PEP/PDP) infrastructure is required or implemented in the MVP. BR-010 (no authentication required for MVP) is unchanged by this section. This group only constrains the code from adopting structural shortcuts — network-based trust, bypassed interfaces, cached authorisation — that would preclude zero-trust adoption later.

Verifiability: at code review, search for IP-based or network-topology-based conditional logic, hardcoded bypass tokens or "trusted" headers accepted without verification, and any code path where one component accesses another's internals without going through a defined interface.

#### Cloud Deployment and Managed Cloud Service Readiness (v1.4)

The MVP remains local-only (Section 7, item 14; Section 1.3 Technology Stack). The requirements below ensure that deploying to a cloud provider (Azure, AWS, or GCP), or consuming a managed cloud service (managed database, secrets manager, blob storage, managed cache), can be done later using the same interface-seam pattern already established for persistence (`IBookingStore`, DP-PERSIST-001–005) and providers (`IFlightProvider`, DP-001).

| ID | Requirement | Principle | Priority |
|---|---|---|---|
| DP-CLOUD-001 | Any future integration with an external managed service (database, secrets manager, blob/object storage, managed cache) must be defined behind an application-owned interface, following the same pattern as `IFlightProvider` (DP-001) and `IBookingStore` (DP-PERSIST-001) — never called directly via a cloud-vendor SDK client type from a service or controller class. For example, a future secrets-manager integration must be accessed through an application-defined `ISecretProvider`-style interface, not by injecting a vendor SDK client (e.g., Azure Key Vault's `SecretClient`, AWS's `AmazonSecretsManagerClient`) directly into a service class. | DIP, OCP | Must Have |
| DP-CLOUD-002 | No service class, domain model, or business logic may reference a cloud-vendor-specific SDK type, namespace, or attribute in its public signature. Cloud SDK usage, if introduced, is confined to infrastructure/adapter classes implementing the application-owned interfaces required by DP-CLOUD-001. | SRP, OCP | Must Have |
| DP-CLOUD-003 | Configuration values needed for a future cloud deployment (connection strings, service endpoints, region, managed-identity settings) must be sourced through the standard .NET configuration abstraction (`IConfiguration`, environment variables, or `appsettings.{Environment}.json`) — never hardcoded as literal values in source code. This costs nothing for the MVP's local-only deployment and is the same mechanism required for cloud deployment later. | OCP | Must Have |
| DP-CLOUD-004 | The application must not assume a local filesystem is available for any data that could move to cloud-managed storage later (e.g., blob storage). The MVP uses no file storage at all; this requirement only constrains that if file storage is introduced, it goes behind an interface (DP-CLOUD-001) rather than direct filesystem calls scattered through service code. | OCP | Should Have |
| DP-CLOUD-005 | The persistence seam (`IBookingStore`, DP-PERSIST-001–005) and the provider seam (`IFlightProvider`, DP-001) are the reference patterns for all future managed-cloud-service integrations: an application-owned interface, a concrete adapter implementation, and a DI registration, with zero changes to consuming service classes. | DIP, OCP | Must Have |

Note on YAGNI: no cloud SDK package (Azure SDK, AWS SDK, GCP client libraries), no managed database, no managed secrets manager, no managed cache, and no blob storage integration is required, configured, or implemented in the MVP. No cloud account, resource group, subscription, or deployment pipeline targeting a cloud provider is created. This group only constrains that if such an integration is added later, it follows the existing interface-seam pattern.

Verifiability: at code review, search the codebase for `using Azure.*`, `using Amazon.*`, `using Google.Cloud.*`, or any other cloud-vendor SDK namespace. None should exist in the MVP. This requirement's full test is architecture-review-time: when a cloud integration is proposed, confirm it is introduced as an adapter behind an application-owned interface rather than a direct SDK dependency in service/domain code.

#### Deployment Agnosticism — 12-Factor Alignment (v1.4)

The application must be deployable locally, through a CI/CD pipeline (e.g., Jenkins or GitHub Actions), on private/on-premises servers, or in the cloud — using configuration/environment changes only, with no source code changes. This section applies 12-factor-app principles as the governing standard.

| ID | Requirement | Principle | Priority |
|---|---|---|---|
| DP-DEPLOY-001 | All environment-specific values (API base URLs, CORS allowed origins, connection strings, feature flags, port numbers) must be externalised to configuration — `appsettings.{Environment}.json` and environment variables on the backend; Angular environment files or runtime-configurable values on the frontend. No environment-specific value may be hardcoded inline in a component, service, or controller. | OCP | Must Have |
| DP-DEPLOY-002 | The backend must not hardcode assumptions about its hosting environment (a specific reverse proxy, a specific OS path separator, a specific hosting model such as IIS-only). ASP.NET Core's documented cross-platform hosting model (Kestrel, with an optional reverse proxy) must be used without environment-specific branching in application code. | OCP | Must Have |
| DP-DEPLOY-003 | The application must be containerisable without source code changes — nothing in the codebase may assume it is not running in a container (no reliance on machine-specific registry keys, no assumption of non-ephemeral local disk beyond what is documented as safe to lose). Building or maintaining an actual container image is not required for the MVP (see YAGNI guard); this requirement only constrains that adding one later requires no code changes. | OCP | Must Have |
| DP-DEPLOY-004 | Build, test, and run commands must be scriptable and non-interactive (e.g., `dotnet build`, `dotnet test`, `ng build`, `ng test --watch=false`) so that any CI/CD system (Jenkins, GitHub Actions, Azure DevOps, or a local terminal) can invoke them identically. No build or test step may require manual/interactive input. | KISS | Must Have |
| DP-DEPLOY-005 | Secrets (API keys, connection strings, future identity-provider client secrets) must never be committed to source control or hardcoded in configuration files checked into git. They must be sourced from environment variables, a secrets manager (DP-CLOUD-001), or local developer-secrets tooling (e.g., `dotnet user-secrets` for local development). This costs nothing for the MVP because no real secret currently exists (BR-010: no auth). | Security | Must Have |
| DP-DEPLOY-006 | Logging output must be written to standard output/error streams or a configurable sink, rather than assuming a specific filesystem log location, so that containerised and cloud-native log aggregation (CI/CD console capture, `docker logs`, cloud log aggregation) works without code changes. | OCP | Should Have |

Note on YAGNI: no Dockerfile, no docker-compose file, no Jenkinsfile, no GitHub Actions workflow YAML, and no actual deployment to any on-premises server or cloud provider is required or created in the MVP. This group constrains the application code to remain deployment-target-agnostic; it does not require building or executing any specific deployment pipeline or artifact.

Verifiability: at code review, search for hardcoded URLs, absolute file paths, or connection strings in source files outside configuration files. Confirm all environment-specific values resolve through `IConfiguration` or Angular environment files. Confirm no build/test script requires interactive input.

#### Explicit YAGNI Guards — Cross-Cutting Extensibility (v1.4)

The following items are explicitly excluded from the MVP. Introducing any of these without Product Owner approval is a scope violation.

| ID | Item Excluded | Reason |
|---|---|---|
| YAGNI-012 | Automated dependency-upgrade tooling (Dependabot, Renovate) or a framework-abstraction/compatibility-shim layer | DP-UPGRADE-001–004 ensure upgrade flexibility through layering, not tooling; no automation is required in MVP |
| YAGNI-013 | A generalised policy engine, rules engine, or policy administration UI for authorisation or business policies | DP-POLICY-001–003 ensure a future policy can be introduced as a named, isolated seam; no engine or config-driven policy store is required in MVP |
| YAGNI-014 | Zero-trust infrastructure (mutual TLS, service mesh, per-request cryptographic attestation, PEP/PDP components) | DP-ZEROTRUST-001–005 ensure no structural precluding assumption exists; BR-010 (no auth) is unchanged for MVP |
| YAGNI-015 | Any cloud-provider SDK, managed database, managed secrets manager, managed cache, or blob storage integration | DP-CLOUD-001–005 ensure future addability via the existing interface-seam pattern; MVP remains local-only (Section 7, item 14) |
| YAGNI-016 | Dockerfile, docker-compose file, Jenkinsfile, GitHub Actions workflow YAML, or any actual CI/CD pipeline configuration | DP-DEPLOY-001–006 ensure the application code does not preclude any of these; none is created in MVP |
| YAGNI-017 | OIDC client, OAuth 2.0 client/server, SAML library, or SSO broker integration | DP-AUTH-005–006 ensure these protocols are addable without service/component changes; none is implemented in MVP |
...
| ASM-016 | The architecture is framework/library-upgrade tolerant by design (DP-UPGRADE-001–004): domain and service layers do not reference third-party or framework-internal types directly. Upgrading Angular, .NET, or key libraries to a new major version is expected to require adapter-layer changes only. | If a future upgrade requires broad domain/service-layer rewrites, a design review must be triggered before proceeding |
| ASM-017 | A general pluggable-policy extension point exists (DP-POLICY-001–003), generalising the auth-policy seam (DP-AUTH-004) to any future authorisation or business policy. No custom policy engine is implemented in MVP; BR-010 (no auth) is unchanged. | If a future custom policy cannot be introduced without modifying existing service method bodies, a design review must be triggered before proceeding |
| ASM-018 | The architecture does not structurally preclude zero-trust principles (DP-ZEROTRUST-001–005): no implicit network-location-based trust, no bypassed interface boundaries, least-privilege interface scoping. BR-010 (no authentication required for MVP) remains unchanged. | If a future zero-trust adoption requires removing hardcoded trust assumptions from existing code, a design review must be triggered before proceeding |
| ASM-019 | The architecture does not preclude cloud deployment or consumption of managed cloud services (DP-CLOUD-001–005), via the same interface-seam pattern used for persistence and providers. No cloud provider, resource, or SDK is used in MVP (Section 7, item 14). | If a future cloud integration requires direct cloud SDK references in service/domain classes, a design review must be triggered before proceeding |
| ASM-020 | The application is deployment-agnostic by design (DP-DEPLOY-001–006, 12-factor style): deployable locally, via a CI/CD pipeline, on-premises, or in the cloud through configuration changes only. No CI/CD pipeline, container image, or deployment target beyond local development is implemented in MVP. | If a future deployment target requires source code changes rather than configuration changes, a design review must be triggered before proceeding |
| ASM-021 | The authentication seam (DP-AUTH-001–006) is explicitly capable of hosting OIDC, OAuth 2.0, SAML, or SSO providers without business-logic or component changes. No such provider is integrated in MVP; BR-010 is unchanged. | If a future identity-protocol integration requires service or component changes, a design review must be triggered before proceeding |
...
| 28 | Automated dependency-upgrade tooling and any actual framework/library major-version upgrade effort | Not required for MVP. DP-UPGRADE-001–004 (YAGNI-012) ensure the architecture is upgrade-tolerant; no actual upgrade is performed. |
| 29 | A custom/pluggable authorisation or business-policy engine, policy administration UI, or config-driven policy store | Not implemented in MVP (BR-010: no auth). DP-POLICY-001–003 (YAGNI-013) ensure a future policy can be added as a named seam without rework. |
| 30 | Zero-trust security infrastructure (mutual TLS, service mesh, PEP/PDP components, per-request cryptographic attestation) | Not implemented in MVP. DP-ZEROTRUST-001–005 (YAGNI-014) ensure the architecture does not structurally preclude zero-trust adoption later. |
| 31 | Cloud provider deployment and managed cloud service consumption (Azure/AWS/GCP compute, managed database, secrets manager, blob storage, managed cache) | Local development only for MVP (see item 14). DP-CLOUD-001–005 (YAGNI-015) ensure future addability via the existing interface-seam pattern used for persistence and providers. |
| 32 | CI/CD pipeline implementation (Jenkins, GitHub Actions, or any other) and containerisation artifacts (Dockerfile, docker-compose) | Not created in MVP. DP-DEPLOY-001–006 (YAGNI-016) ensure the application remains deployable via any pipeline or target through configuration changes only. |
| 33 | OIDC, OAuth 2.0, SAML, or SSO identity-provider integration | Not implemented in MVP (BR-010: no auth). DP-AUTH-005–006 (YAGNI-017) ensure any of these protocols can be added later without business-logic or component changes. |
