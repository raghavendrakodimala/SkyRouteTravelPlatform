...
| Version | 1.0 |
| Date | 2026-07-03 |
| Status | Approved — Numeric Targets Confirmed by Human Product Owner 2026-07-03 |
| Owner | solution-architect |
| Approvers | Product Owner (numeric targets confirmed 2026-07-03 — see Section 17) |
...
- **Target / Measurement** — a measurable threshold where one is meaningful; marked **[CONFIRMED by Human PO 2026-07-03]** where the specific number was new information introduced by this document (i.e., not already stated in `docs/requirements.md`) and has since been reviewed and accepted — see Section 17.
...
| ID | Requirement | Priority | Target / Measurement | Validation Method | Traceability | Architecture Link |
|---|---|---|---|---|---|---|
| NFR-COMPAT-001 | The frontend must run correctly on the latest 2 major versions of each evergreen browser: Chrome, Edge, Firefox, and Safari. | Must Have | Manual smoke test of the core flows (search → results → booking → confirmation) passes on at least Chrome (latest) and one additional evergreen browser (Edge or Firefox) before Phase 14 sign-off; Safari verification is best-effort if a macOS test environment is available **[CONFIRMED by Human PO 2026-07-03 — specific minimum browser-pair for MVP sign-off, since exhaustive 4-browser × 2-version manual testing is disproportionate for MVP scope; requirements.md §5.10 states the target browser set but not a minimum verified subset]**. | Manual cross-browser smoke test (Phase 14). | requirements.md §5.10 | — |
...
## 17. Summary — Numeric Targets Confirmed by Human Product Owner (2026-07-03)

The following numeric thresholds were **new information** introduced by this NFR specification — not already stated in `docs/requirements.md`. They were presented to the Human Product Owner on 2026-07-03 and **accepted as proposed, with no adjustments**. They are now treated as enforced gates at later phases (Test Strategy, Code Review, Test Execution).

| # | NFR ID | Confirmed Target | Rationale | Status |
|---|---|---|---|---|
| 1 | NFR-PERF-001 | p95 < 2000 ms / p50 < 800 ms, percentile-based measurement over ≥20 sequential local requests | Requirements.md states "under 2 seconds" without specifying percentile or sample size | Confirmed 2026-07-03 |
| 2 | NFR-PERF-002 | p95 < 1000 ms / p50 < 400 ms | Requirements.md states "under 1 second" without percentile | Confirmed 2026-07-03 |
| 3 | NFR-PERF-003 | Sort re-render < 100 ms for result sets up to 50 flights | Requirements.md/US-003 says "no perceptible delay" without a number | Confirmed 2026-07-03 |
| 4 | NFR-PERF-004 | Airport dropdown population < 50 ms | New, minor, low-risk | Confirmed 2026-07-03 |
| 5 | NFR-PERF-005 | Aggregation overhead < 100 ms above slowest provider | New — operationalises requirements.md §5.2's qualitative "should not degrade" statement | Confirmed 2026-07-03 |
| 6 | NFR-TEST-005 | 80% line coverage target for backend service-layer classes | New — requirements.md §5.8 requires unit-testability but sets no coverage percentage | Confirmed 2026-07-03 |
| 7 | NFR-COMPAT-001 | Minimum verified browser pair for Phase 14 sign-off (Chrome + one of Edge/Firefox; Safari best-effort) | Requirements.md names 4 browsers × latest-2-versions without specifying a minimum verified subset for MVP timeline | Confirmed 2026-07-03 |
