...
test.describe('Passenger validation — in-place form (US-005 AC10)', () => {
  test('invalid add/confirm keep the form open with inline errors; edit, cancel edit, and remove allow correction', async ({
    ...
    page.on('request', (req) => {
      if (req.method() === 'POST' && req.url().includes('/api/v1/bookings')) {
    ...
    // Fix the name. Age (PO age feature 2026-07-08, DEC-022): a blank age blocks the add
    // with the range/required message; an out-of-range value is equally rejected. No
    // lead-adult rule exists — age is pure data capture with 0–120 sanity bounds.
    await fullName.fill('Jane Doe');
    await age.fill('121');
    await clickAddAnotherPassenger(page);
    await expect(savedCards).toHaveCount(0);
    await expect(page.getByText('Age is required and must be a whole number between 0 and 120.')).toBeVisible();
    await expect(age).toBeFocused();

    // Fix the age; a malformed email still blocks the add, focus moves to the email field.
    await age.fill('34');
