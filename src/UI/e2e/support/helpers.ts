 ...
 *
 * Route filtering + route coverage (ProviderScheduleMapper.BuildResults +
 * RouteScheduleGenerator, PO defect fix 2026-07-07): providers return only schedule entries
 * matching the requested origin/destination (case-insensitive), and every ordered pair of the
 * 6 supported airports returns flights — the documented fixtures on fixture routes (e.g.
 * LHR -> JFK stays exactly GA101 + BW210), deterministic generated flights everywhere else —
 * with ONE deliberate exception: MAN <-> SYD is the DEC-021 (PO 2026-07-08) no-direct-service
 * route, empty in both directions from both providers, so the styled empty state stays
 * reachable through a real search (exercised un-intercepted in `e2e/error-states.spec.ts`).
 * Route/network interception is used only in `e2e/error-states.spec.ts` for the two 5xx
 * response shapes (still not producible through valid UI input) and in
...
export const API_SEARCH_URL = '**/api/v1/search';
export const API_BOOKINGS_URL = '**/api/v1/bookings';
...
export interface PassengerInput {
  fullName: string;
  /** Whole number 0–120 (PO age feature 2026-07-08; sanity bounds only — no business rule
   * is bound to age, DEC-022); defaults to 34 when omitted. */
  age?: number;
  email: string;
 ...
 * NEW passenger the index equals the number of already-saved passengers). Field order:
 * Full Name, Age (default 34), Email, Document Number.
 */
export async function fillActivePassengerForm(page: Page, index: number, passenger: PassengerInput): Promise<void> {
  ...
  await page.locator(`#age-${index}`).fill(String(passenger.age ?? 34));
