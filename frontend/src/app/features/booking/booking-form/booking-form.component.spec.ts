...
describe('BookingFormComponent', () => {
  ...
  /** Fills the single active form (whose input ids carry `index`). */
  ...
  it('Confirm with a filled form validates and saves it, then submits ALL saved passengers', async () => {
    ...
    expect(fakeBookingState.submitBooking).toHaveBeenCalledWith({
      ...
      passengerCount: 2,
      passengers: [
        { fullName: 'Passenger 1', email: 'p1@example.com', documentType: 'Passport', documentNumber: 'AB123451' },
        { fullName: 'Passenger 2', email: 'p2@example.com', documentType: 'Passport', documentNumber: 'AB123452' },
      ],
    });
  });
  ...
  it('Confirm with an INVALID filled form saves nothing and does not submit (same invalid handling as add)', async () => {
    addPassengers(1);
    setInput('fullName-1', 'Only A Name'); // email + document missing
    ...
    expect(qa('.error').length).toBeGreaterThan(0);
    expect(document.activeElement?.id).toBe('email-1');
  });
  ...
  it('back-nav after confirmation shows the BOOKED total and count from the response (CR-003)', () => {
    ...
    response.passengers = [{ fullName: 'P One' }, { fullName: 'P Two' }, { fullName: 'P Three' }];
