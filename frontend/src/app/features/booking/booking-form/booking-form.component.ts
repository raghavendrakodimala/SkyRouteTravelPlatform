...
const PASSENGER_ERROR_KEY = /^passengers\[(\d+)\]\.(fullName|email|documentNumber|documentType)$/;

const FORM_FIELD_ORDER = ['fullName', 'email', 'documentNumber'] as const;
...
export class BookingFormComponent {
  ...
  protected readonly parkedDraft = signal<SavedPassenger | null>(null);
   ...
   * forms revalidate identically (label + validation switch together, BR-003). */
  ...
  protected async onConfirm(): Promise<void> {
    ...
    // freezes the array while the request is in flight, so index mapping holds.
  ...
  private activeFormHasInput(): boolean {
    const raw = this.activeForm.getRawValue();
    return raw.fullName.trim() !== '' || raw.email.trim() !== '' || raw.documentNumber.trim() !== '';
  }
  ...
  private resetActiveForm(): void {
    ...
    focusedControl?.blur();
    this.activeForm.reset({ fullName: '', email: '', documentNumber: '' });
    this.activeFormDirty.set(false);
  }
