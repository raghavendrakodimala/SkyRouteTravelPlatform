...
- **Booking flow**: `BookingRequest` carries a full flight-detail snapshot (provider, flight number, route, times, cabin class, price) rather than an opaque flight ID, since the mock providers have no independent lookup store to match against. On submission, `BookingService` re-resolves the route type and re-derives the true fare server-side from the same provider schedule the original search used, rather than trusting the client-submitted price outright, and rejects the booking if the two disagree. Booking records are persisted through `IBookingStore` by `EfCoreBookingStore` — EF Core (SQLite) over a kept-open in-memory database whose schema (`Bookings` + `BookingPassengers`) enforces the DATA-MODEL-001 constraints (PK, NOT NULLs, and CHECKs) at the database boundary; the interface is swap-ready to a durable SQL Server/PostgreSQL database via a connection string with no service/controller change (DEC-025). The original `InMemoryBookingStore` is retained behind the same interface as the alternative implementation.
...
- **Real schema, non-durable default store.** Bookings are persisted via EF Core over SQLite with a real constraint-enforcing schema (`EfCoreBookingStore`; PK/NOT NULL/CHECK constraints per DATA-MODEL-001), but the default MVP database is SQLite `:memory:` (a single kept-open connection), so booking data still does not survive an API process restart. Supplying a `ConnectionStrings:Bookings` value switches to a durable database (file SQLite today; a one-line `UseSqlServer`/`UseNpgsql` change plus its package for SQL Server/PostgreSQL) with no change to `BookingService`, any controller, or the domain — the `IBookingStore` seam isolates the swap (DEC-025). The prior `InMemoryBookingStore` is retained behind the same interface as the alternative implementation.
...
│   ├── SkyRoute.Infrastructure/        Mock providers, EF Core (SQLite) + in-memory booking stores, tenant context
│   └── tests/
